...
- **`guild`** — **curated-only** (the 72). NULL for uncurated birds and non-birds in `ref_species_traits`. Never derive a blended guild from AVONET. On the `species.guild` *display* projection only: curated value where present, else the AVONET `trophic_niche` as a grounded diet label (never a guessed stratum).
