...
**Migration state:** `main` and prod are **in sync through `20260727`**. `20260724` node_banner (`nodes.banner_path` + `node-banners` bucket + `set_node_banner` owner-checked SECURITY DEFINER RPC) and `20260725` scope_service_role_policies (**security remediation** — see below) were applied 2026-07-31 via `supabase db query --linked` in an explicit transaction. `20260726` drop_legacy_users_table removed the empty legacy `public.users` (NOT `auth.users` — distinct objects, oid 18591 vs 16499; auth verified intact after). `20260727` ingest_node_id_constraints (**cascade remediation** — see below) applied 2026-08-04, same method. **Next free migration is `20260728`.**

**Cascade remediation (`20260727`, 2026-08-04).** `detections.node_id` and `aci_logs.node_id` carried `ON DELETE **CASCADE**` foreign keys to `nodes(id)` — deleting one node row would have silently erased its entire record (194,508 detections + 164,369 aci_logs at the time). `20260725` named node DELETE "the worst case" and listed the cascading dependants, but its list **omitted `detections` and `aci_logs` — the two tables that ARE the record.** A Phase 0 investigation subsequently reported these columns had "no FK" at all, which was also wrong; the gap was never missing referential integrity but the wrong referential *action*. Fixed by dropping and re-adding both as `ON DELETE RESTRICT`, plus `NOT NULL` on both `node_id` columns. Caller audit first (per the `20260721` precedent): nothing in the codebase deletes `nodes` rows — decommissioning is `is_active = false`, as birdnode11 was handled — so nothing depended on the cascade. Verified after: NULL `node_id` → 23502, orphan `node_id` → 23503, deleting a node with rows → 23503 RESTRICT, valid insert still succeeds. **Rule: read `pg_get_constraintdef` before asserting what a foreign key does; a constraint's existence says nothing about its delete action.**
...
- **Never `VITE_`-prefix a secret.** In Vite the `VITE_` prefix *means* "inline this into the client bundle" — it is an opt-in to publication, not a naming convention. `VITE_PROVISION_SECRET` shipped the provisioning gate secret in plaintext inside `/assets/index-*.js` for ~50 days (found and rotated 2026-08-02; see `docs/tasks/provision-secret-rotation-runbook.md`). Server-side secrets take a bare name and are read via `process.env` in an `api/` function. This is the same failure shape as `20260725`: **a name that implies a gate the mechanism does not provide.** Verify by grepping the built `dist/` for the value — never by reading the variable's name.
- **Fetch repo head before scoping; treat a TaskDef's stated state as suspect until confirmed.** `magora-acoustic-biodiversity` moves independently of Chat's context, and Chat cannot read either repo. This drift bit twice in one week: 2026-08-03, an inventory run against a cached clone duplicated USB-detection work already merged (discarded in favour of the better existing implementation, preserved on `backup/superseded-usb-work`); 2026-08-04, a TaskDef header named `cfea5b7` and "next free `20260727`" when `main` was four commits further on and `20260727` was already applied. **Code:** `git fetch` both repos as the first action of any inventory, and re-verify claimed prod state (`pg_constraint`, `pg_attribute`) rather than inheriting it. **Architect:** state repo/ledger positions as assumptions to be confirmed, never as givens.
- **External builders fetch from `release`, never `main`.** Everything a stranger's node pulls at first boot (`birdnet.service`, and `BUILD.md` for the rendered guide) comes from the `release` branch of `magora-acoustic-biodiversity`; `detect.py` is pinned harder still, to an exact `DETECT_SHA`. Bare `main` couples every unverified commit to someone's first boot and makes a failure unreproducible once `main` moves; a frozen SHA rots instead. `release` advances only after the walkthrough is re-verified against it. Runbook: `docs/tasks/release-pointer-runbook.md`.
- **The Management API `/database/query` endpoint is READ-ONLY.** It executes DDL, returns success, and persists nothing. It silently reported OK for `20260727` twice while the catalog was unchanged. Use it for inspection only; apply all DDL with `supabase db query --linked` and **verify against `pg_constraint` / `pg_attribute` afterwards** rather than trusting the response.
