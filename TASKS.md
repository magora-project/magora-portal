...
- **Seasonal-native recovery — INVESTIGATED July 12, deferred (gate kept tight; NO change).** Seasonal/elevational natives now hidden (Gray-crowned Rosy-Finch, Varied Thrush, Bohemian Waxwing). Options weighed: (a) true **week-resolved** allowlist — eBird's weekly barchart frequency is **HTML-only** (no free API; Status & Trends is a heavy separate dataset), not cheaply feasible. (b) **county-annual union** (`product/spplist/{county}`) — simulated: recovers the natives BUT its dominant visible effect is re-injecting **high-count vagrant-misIDs** on the county list (Chestnut-sided Warbler ×196, Nashville Warbler ×41) that would rank near the top of "most recorded" — undercutting the gate's whole purpose. **Not shipped.** (c) **Right fix (future task):** a **frequency-filtered regional list** — sample eBird `data/obs/{region}/historic/{y}/{m}/{d}` across the year to keep *regularly-occurring* species (rosy-finch/waxwing recovered) while excluding *one-off vagrants* (Chestnut-sided Warbler stays hidden). Real work (~hundreds of eBird calls to reconstruct frequency); its own session.
