...
- 🟢 **NodePage narrator (voice) picker REMOVED — SHIPPED July 28 2026 (`9e9638e`, no migration).** The NodePage Pulse narrative now always renders in the `node` voice (the place's own words); the reader-facing selector (node/attenborough/comedy/data_scientist) is gone. `usePulseNarrative` hard-selects `'node'`; check-before-generate on the `(pulse_id, 'node')` cache unchanged (hit+miss verified live). The voice registry / `pulse_narratives` / `(pulse_id, voice)` key / render RPCs are **retained as substrate** (report-model voices reuse them). Labels: Pulse panel → "In this place's own words"; the phenology-report card → "This place, over time" (de-dup). Narrative Agent / Pulse payload / routing / schema untouched; Elder/IEK still excluded.

- 🟢 **Report Share Cards v1 (OG unfurl) — SHIPPED to prod July 17 2026 (`549af4b`, no migration).** A pasted report permalink now auto-unfurls with a generated node-voice card (the closing open-wondering as the hero line) in Slack / iMessage / Twitter — the growth lever the permalink opened. `api/og-report.js` (`@vercel/og` edge, ~1200×630, Satori object-form so it's a `.js` fn Vercel detects) + `api/report-page.js` (a `vercel.json` internal rewrite of the report route → injects per-report `og:`/`twitter:` meta into the SPA shell; crawlers read meta, users still boot the app) + a versioned `og:image` URL keyed on `generated_at` (platforms cache by URL, so a regen re-unfurls). Grounded in the label-fixed report (no anthropophony as a voice, corrected tallies); no person data; +2 functions (16, within Pro). Verified live daily/seasonal/annual + cache-bust. **Deferred fast-follow:** downloadable 1:1/9:16 crops off the same pipeline. *(Log: Build-Log-July2026-Report-Share-Cards-v1.)*

- 🟢 **Node Phenology Report bundle — SHIPPED to prod July 17 2026 (migration `20260723`).** The story-over-time surface: a node writes its own first-person report of its ecological activity, DB-grounded, node-voice, on **Sonnet**, ending on an open wondering. **The rendered reader that un-blocks Pulse's batch half.** Three ships, all on `main` no-squash (`de398e6` → `4681a58` → `ea05739`):
  - **v1 (daily spine)** — `api/_report/{payload,narrate-report,db,generate}.js`, `api/report-ondemand.js`, NodePage card + permalink `/node/:id/report/:period_key`. Migration `20260723` (`node_reports` cache, `set_node_report` service_role-only write, public read).
  - **v1.1 (seasonal + annual + batch cron)** — no migration (jsonb payload). Hemisphere-aware meteorological seasons; per-species arrivals/departures + week-over-week (seasonal); seasonal transitions + milestones (annual). `api/report-cron.js` + 3 `vercel.json` crons (daily `0 10`, seasonal `0 11 1 3,6,9,12`, annual `0 12 1 1`). Daily cron runs `pulseBatch` (alert suppressed via `opts.suppressAlert`) then the report; seasonal/annual **read** accumulated daily pulses (never `pulseBatch` on a long window). One `[report ops]` digest/run.
  - **Label-quality fix** — no migration. `api/_report/labels.js` `classifyLabel` → bird | biophony | anthropophony (signal: `ebird_code`=bird, binomial `scientific_name`=biophony, curated anthropophony set for BirdNET noise classes, ambiguous→biophony). Anthropogenic noise ("Human non-vocal", engines) partitioned into a `human_activity` context field, no longer narrated as a voice; insect biophony (katydids) stays a voice.
  - **Deploy needed Vercel Pro** (report crossed the Hobby 12-function cap; `api/` now 14). Verified live on birdnode11 (daily round-trip cached; seasonal/annual regenerated; cron 3/3/0-errors idempotent; noise reframed, tally 136→135). Fast-follows: stylized report roster, image share cards, set `SLACK_WEBHOOK_URL`.

- 🟢 **Range-Gate Precision Fix v1 — SHIPPED to prod July 12 2026 (migration `20260722`).** The gate was over-quarantining (found during the completion ship). Fixed: (a) `is_plausible` now **fails open (`null`) for species with no valid `ebird_code`** — checked AFTER allowlist membership, so non-birds/insects and null-code birds are no longer quarantined; (b) `ebird_code` repaired for the taxonomy-drift locals (Yellow Warbler `manwar1`→`yelwar1`, Warbling Vireo `eawvir1`→`wewvir2`); (c) `api/range-allowlist-build.js` made split/lump-resilient (curated `SPLIT_ALIASES` + alias→sci→common→code order) **and a latent bug fixed** — `speciesLookups` didn't paginate, so ~166 of 1166 species (incl. Warbling/Yellow Warbler) were invisible to every prior build. Allowlist rebuilt (10 cells). **After-dry-run:** detections quarantined **20,668 → 12,909** (the 7,678 non-bird/null-code rows now fail open; the named locals now `plausible`); **0 non-birds quarantined**; remainder is ~only true exotics (albatrosses, the Eurasian suite, Rook). Idempotent (2nd rebuild identical). **Flag still NOT consumed** — Part A stays reverted, backfill stays dry-run — pending the go-ahead to re-enable.
- 🟢 **Flag #1 (null-code coding) — SHIPPED July 12 2026.** `scripts/backfill-null-species-codes.mjs` filled `ebird_code` on **773** null-code bird rows so the gate can now JUDGE them — drift-proof (exact eBird species-level sci match only; 46 non-birds/lumped species correctly stay null → fail open; Pacific-slope Flycatcher excluded as a `wesfly` lump). Allowlist rebuilt. **After-dry-run:** `unchecked` collapsed **7,678 → 1,399** (only true non-birds/drifted remain fail-open); quarantined **12,909 → 19,192** as null-code exotics are now caught — Gentoo Penguin, Hawaii Amakihi, Hooded Oriole, the Eurasian suite now `false`; locals (Yellow Warbler, Cordilleran) still `true`; non-birds still `null`. 0 current local-risk. Data backfill (no migration); re-runnable/idempotent.
- 🟢 **Re-enable consumption — SHIPPED + VERIFIED LIVE July 12 2026 (the original problem is now closed end-to-end).** Part A (read-side hide) re-applied (commit `67b44e4`, redeployed) and the historical backfill run (`scripts/range-backfill.mjs --apply`): detections **95,471 → plausible 74,879 / quarantined 19,193 / unchecked 1,399** (raw rows kept, only `range_status`/`range_checked_at` changed). Verified against prod on the anon key: Casa Colibri's "most recorded here" now shows only locals (Broad-tailed Hummingbird, American Crow, House Finch, Pine Siskin…); **Rook shown 0 / raw 522**, **Gentoo Penguin shown 0 / raw 2** — the penguin + Rook are gone from the page but still queryable. Backfill hardened to finish a 95k-row table in one pass (memoize `is_plausible` by species+coords not week; fan PATCHes 25-wide) after two timeouts.
- 🟢 **Confidence-floor retune — REVIEWED July 12, NO CHANGE warranted (`MIN_CONFIDENCE` stays 0.30).** Diagnostic over all 95,517 detections (confidence × `range_status`): (1) range false-positives are **not** cleanly low-confidence — 762 quarantined rows sit at ≥0.8 (BirdNET is often confident about a Rook), so a floor never catches them; the **range gate is the right tool** and already does. (2) Since the gate hides every quarantined row regardless of confidence, a floor bump's entire effect now lands on the **shown (legit) set** — 0.30→0.40 would additionally hide **14,738 real detections (32%)**, →0.50 hides 51%. The retune's premise ("FPs are also low-confidence") is **superseded by the range gate**; raising it would cut the ecological record, not noise. (Parallels the Task E "investigated, not viable" outcome.)
- **Seasonal-native recovery — INVESTIGATED July 12, NOT VIABLE via eBird obs data; gate kept tight (NO change).** Seasonal/elevational natives now hidden (Gray-crowned Rosy-Finch ×51, Varied Thrush ×142, Bohemian Waxwing). Three eBird approaches tested + rejected: (a) true **week-resolved** allowlist — eBird weekly barchart is **HTML-only** (no free API). (b) **county-annual union** (`product/spplist`) — recovers the natives but re-injects **high-count vagrant-misIDs** on the county list (Chestnut-sided Warbler ×196) that would top "most recorded". (c) **frequency-filtered regional list** (sample `data/obs/{region}/historic/…`) — **tested (48 samples/2yr, Park County MT): the target natives are as rare in eBird as the vagrant-misIDs** (rosy-finch 1/48, varied thrush 0/48, redpoll 0/48 vs Chestnut-sided Warbler 0/48) — they're irruptive high-alpine/winter specialists, sparse in county-wide effort, so **no threshold separates them from noise.** ⇒ Recovery would need eBird **Status & Trends** modeled abundance (heavy separate dataset) or **manual per-node "known local" curation** — neither a quick follow-up. Accepting the small loss (a few hundred hidden native detections) is the right call for now.
- **Confidence-floor retune — REVIEWED, no change** (see above). **Pulse branch reconcile — RESOLVED July 12:** `main` already contains the Pulse feature (migrations `20260711`/`20260712` byte-identical; `api/_pulse/*` is a superset — the branch code plus v1.1 + range-gate work). `feat/pulse-agent-v1` (+ `feat/pulse-v1.1-per-surface-selection`, `docs/architect-charter-and-pulse-taskdef`) are **stale/superseded** and safe to delete from origin. Only standing item left: set `SLACK_WEBHOOK_URL` (operator secret; Vercel dashboard).
- _**Prod state:** `20260721` (hardening) applied ✓ + Part C deployed ✓. `20260722` (precision fix) applied ✓. Flag #1 null-code coding applied ✓. **Part A (read-side hide) LIVE ✓** + **historical backfill applied ✓** — range gate is fully consumed on prod, the penguin/Rook are hidden. Repo `main` = `origin/main` = prod._
...
- [x] **Range-Gate Completion v1 — node-read consumption + historical backfill + hardening** (July 2026)
  - Closes the two seams the go-forward gate (`20260720`) left open, so the penguin / Rook range false-positives actually disappear from where people look. Raw is always kept (flag-and-quarantine, never delete). **Fail-open preserved throughout:** only rows explicitly `range_status = 'quarantined'` are hidden; `'plausible'`/`'unchecked'`/null all pass. No uncovered (unseeded) cell is quarantined by any of this.
  - **Part A — read-side consumption (no DDL).** Every `detections` read for node/public display or Pulse scoring now excludes quarantined rows null-safely (`range_status IS DISTINCT FROM 'quarantined'`, expressed as `or=(range_status.is.null,range_status.neq.quarantined)` — never `= 'plausible'`, which would wrongly hide unchecked/null and break fail-open). Surfaces: NodePage recent-list + "most recorded here"/species stats; SpeciesPage aggregate; MapPage live feed + "species today"; **all five Pulse candidate reads** in `api/_pulse/sources.js` (so a penguin can't drive `novel_detection`/`activity_spike`/`survey_gap`); and the `api/insight.js` narrative-context reads (co-occurring / diversity). The `public_mobile_detections` view was already filtered by `20260720`.
  - **Part B — historical backfill (no DDL): `scripts/range-backfill.mjs`.** Idempotent, batched (keyset-paginated), service-role script that walks existing `detections` + `mobile_detections`, calls the gate's own `is_plausible()` per row (memoized per cell/species — the 106 Rook rows cost 1 RPC), and sets `range_status`/`range_checked_at`. Mirrors each trigger's own lat/lon + primary-species resolution; PATCHes only on change so a second run is a no-op. Respects the coverage guard: uncovered cells resolve to `'unchecked'`, never quarantined. Dry-run by default; `--apply` to write. **Operator step — not yet run against prod.**
  - **Part C — harden `record_node_status` (migration `20260721`).** Revokes `anon`/`authenticated`, grants `service_role` only — closes the unauthenticated write path into `node_status_events` and clears one `absence` precondition (parallel to `apply_species_trait`/`replace_range_cell`). **Caller audit first:** the sole caller (`api/node-status-check.js` via `_node_status/db.js`) wrote on the **anon** key, so hardening blind would have broken node liveness — its write path (`sbRpc`) was switched to the service role key in the same change (reads stay on anon). Grant-only migration; no data touched.
  - **Verified:** lint clean (no new errors), `vite build` green, backfill script parses + dry-run-safe. Post-apply acceptance (operator): penguin + Rook gone from NodePage/"most recorded"/species aggregates and Pulse-ineligible while raw rows stay queryable; locally-plausible species unaffected; `record_node_status` works under service_role, rejected for anon, node liveness still updates.
  - **Out of scope (fast-follows):** confidence-floor retune; allowlist coverage beyond the 8 seeded cells (monthly cron grows it); `SLACK_WEBHOOK_URL` operator config.

- [x] **Universal Range-Validity Gate v1 — place-keyed plausibility gate for ALL detections** (migration `20260720`, July 2026)
  - BirdNET posts geographically impossible species (Rook ×106, Carrion Crow, penguins) because nothing checked regional plausibility between inference and publish. Adds that check as ONE universal, place-keyed primitive applied to both the node path (`detections`) and listen-post path (`mobile_detections`), keyed on each place's ~0.5° coordinate cell + week — never a per-named-node allow-list. New places inherit it automatically once they have coordinates.
  - **Primitives:** `range_cell_key`/`range_week` (shared keying), `range_allowlist` ref table (anon-read; writes only via the `service_role`-only `replace_range_cell` RPC), `is_plausible(species_id, lat, lon, week)` tri-state gate (true→plausible / false→quarantine / null→fail-open), and BEFORE-INSERT triggers on both tables that flag + quarantine (raw kept — parallels the `mobile_detections`→`public_mobile_detections` consent gate). Allowlist builder: `api/range-allowlist-build.js` (eBird-sourced, monthly cron).
  - **Coverage guard (commit `9078f7d`):** the `species` table is North-America-biased, so a low-coverage cell would produce an incomplete allowlist that over-quarantines legit local species. The builder refuses cells below `MIN_CELL_SPECIES` (25) / `MIN_CELL_COVERAGE` (0.4) — they stay allowlist-free and `is_plausible` fail-opens there (EIA §8: an unimplemented region goes quiet, never wrong). Live seed: 8 well-covered cells built (Casa Colibri 156 species), 3 low-coverage tropical cells skipped. Verified on prod: penguin/Rook@casa false, local true, penguin@tropical null.
  - **Go-forward only at ship** — the read-side consumption + historical re-scan landed in **Range-Gate Completion v1** (above).

- [x] **Elevation unit disambiguation** (migration `20260719`, July 2026)
  - `nodes.elevation_m` is named "meters" but every value was actually feet (e.g. 6300 ft), and nothing recorded the unit — so consumers trusting the column name misread it, including `api/insight.js` feeding "Elevation: 6300m" into the narrative prompt and distorting montane-species plausibility. Added `nodes.elevation_unit` (NOT NULL DEFAULT 'ft', CHECK in ('ft','m')); values kept exactly as-entered (no canonicalization). Display (NodePage/SpeciesPage/MapPage) and the insight prompt now render the stored unit. Node-only (mobile carries no elevation); RLS inherited.

- [x] **Voice roster v1.1 — reader-selectable narrative voices** (PR #10, July 2026)
  - A small registry of reader-selectable voices (node / attenborough / comedy / data_scientist) on the Narrative Agent. Every voice renders the SAME canonical PulsePayload on the SAME on-demand NodePage "Pulse" panel — only register/tone varies, never the facts. `api/_narrative/voices.js` (registry + `getVoice` + `VOICES_VERSION`); `narrate.js` assembles the prompt in 3 fixed segments (grounded scaffold / voice directives / invariant rules) — facts live only in the scaffold, voice restyles and never adds. Elder/IEK is deliberately NOT a member (gated on the IEK consent model, not a render config); `getVoice('elder')` resolves to null, hard-rejected with no silent fallback. No new surface, no Pulse-scoring change, no person data, no Slack path.
