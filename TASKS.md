...
- 🔴 **Range gate needs a PRECISION fix before its flag can be consumed (found July 12 2026, during the completion ship).** Shipping Range-Gate Completion v1 to prod surfaced that the eBird allowlist **over-quarantines legitimate species**: a dry-run of the historical backfill would flag **6,932** detections, of which many are legit — **all non-birds** (katydids/insects have no eBird range, so they can never be in a bird allowlist) and **local birds lost to eBird taxonomy drift** (Warbling Vireo, Yellow Warbler [mis-mapped `ebird_code` = *Mangrove* Warbler], Cordilleran Flycatcher [lumped into "Western Flycatcher", null code]). So the gate is **not precise enough to drive a hide**. Fix before re-enabling consumption: (a) `is_plausible` should **fail-open (`unchecked`) for non-birds / species with no eBird taxonomy**, not quarantine them; (b) repair the eBird species↔allowlist mapping (splits/lumps + the Yellow Warbler code) so legit locals land in the allowlist; then re-verify a dry-run quarantines ~only the true exotics before re-shipping Part A + the backfill.
- _**Prod state after the completion ship:** `20260721` (`record_node_status` hardening) **applied** ✓ and the service-role caller **deployed** ✓ (Part C — good, kept). **Part A (read-side hide) was REVERTED** (partial code revert, redeployed) because it was hiding the false-quarantines above; a full Vercel rollback was NOT used (would reintroduce the anon node-status caller against the now-hardened RPC). **Historical backfill NOT applied** — halted at the dry-run checkpoint; `scripts/range-backfill.mjs` is idempotent/re-runnable, so re-run it after the precision fix. Follow-ups: set `SLACK_WEBHOOK_URL`; reconcile Pulse's `feat/pulse-agent-v1` branch → `main`._
