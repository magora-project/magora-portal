...
- **Next: re-enable consumption** — re-ship Part A (read-side hide) + run `scripts/range-backfill.mjs --apply`, now that the gate is precise and null-code exotics are caught. Remaining trade-off: **seasonal/elevational natives** not in the 30-day allowlist window lose fail-open (e.g. Gray-crowned Rosy-Finch — an alpine local near birdnode11 — now `false`; Varied Thrush) — a week-agnostic-v1 limitation, fixable only by a week/elevation-resolved allowlist. Also still: **confidence-floor retune**.
- _**Prod state:** `20260721` (`record_node_status` hardening) applied ✓ + service-role caller deployed ✓ (Part C). **Part A (read-side hide) REVERTED** ✓ (still reverted — do not re-enable without the go-ahead). `20260722` precision fix applied ✓. Backfill NOT applied. Follow-ups: set `SLACK_WEBHOOK_URL`; reconcile Pulse's `feat/pulse-agent-v1` branch → `main`._
