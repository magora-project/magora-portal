...
- **Next: re-enable consumption** — re-ship Part A (read-side hide) + run `scripts/range-backfill.mjs --apply`, now that the gate is precise. Known trade-off to weigh first: exotic birds with a **null `ebird_code`** (e.g. Gentoo/King Penguin, Hooded Oriole, Scaly-breasted Munia) now fail open too, so they'd show rather than hide — backfilling their eBird codes (same mechanism as the drift repair) would let the gate judge them. Also still: **confidence-floor retune**; seasonal false-quarantines (e.g. Varied Thrush — a winter visitor missed by the 30-day summer allowlist window) are a week-agnostic-v1 limitation.
