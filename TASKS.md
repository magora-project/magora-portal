...
- _Nothing actively in progress. The **Range-Validity Gate** (go-forward gate `20260720` + coverage guard) and **Range-Gate Completion v1** (node-read filtering + historical backfill + `record_node_status` hardening `20260721`) both shipped July 2026 (see Done), alongside earlier July ships — Pulse Agent v1, iNaturalist Step 0, Node Offline Detection v1, elevation-unit disambiguation (`20260719`), voice roster v1.1. birdnode11 recovered (back online)._
- _**Pending operator steps** (code committed; these are manual/service-role actions): apply migration `20260721` to prod; run the historical backfill once under service_role — `node scripts/range-backfill.mjs` (dry run) then `--apply` (the penguin + ~106 Rook rows only disappear from history after this runs). Follow-ups: set `SLACK_WEBHOOK_URL` to light up `[node ops]`/`[pulse ops]`/`[detection ops]` alerts; **confidence-floor retune** (the range false-positives are also low-confidence — separate tuning review). Housekeeping: reconcile Pulse's `feat/pulse-agent-v1` branch → `main` (prod is running it out-of-band)._
