...
- **Remaining follow-ups (none blocking):** **seasonal/elevational natives** outside the 30-day allowlist window are now hidden (e.g. Gray-crowned Rosy-Finch — alpine local near birdnode11; Varied Thrush) — a week-agnostic-v1 limitation, fixable only by a week/elevation-resolved allowlist. Also: **confidence-floor retune**; set `SLACK_WEBHOOK_URL`; reconcile Pulse's `feat/pulse-agent-v1` branch → `main`.
