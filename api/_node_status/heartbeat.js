// Node Offline Detection v1 — heartbeat model (pure derivation, no I/O).
//
// Node liveness is derived from aci_logs — the continuous per-cycle soundscape log, written
// every cycle even at zero birds (verified on prod). A dark node stops writing aci_logs, so
// the gap since the last aci_log is the liveness signal.
//
// The expected cadence is PER-NODE, derived from that node's OWN recent aci_logs history
// (median inter-log interval) — never a hardcoded interval, no region/config assumption. This
// keeps detection global-ready: a node cycling every 24s and a node cycling every 5min are
// each judged against their own rhythm.
